...
const optionsSchema = {
  ...
  description:
    "The options for an instance of the Iconduit Webpack HTML plugin",
  type: "object",
...
module.exports = function IconduitWebpackHtmlPlugin(options = {}) {
  ...
  this.apply = (compiler) => {
    ...
    const childCompilerName = buildChildCompilerName(
      context,
      chunkName,
      manifestPath,
    );
    ...
    async function handleMake(compilation) {
      if (HtmlPlugin.getHooks) {
        HtmlPlugin.getHooks(compilation).alterAssetTagGroups.tapPromise(
          PLUGIN_NAME,
          handleAlterAssetTagGroups,
        );
      } else {
      ...
      const {
        outputOptions: { publicPath: optionsPublicPath },
      } = compilation;
      ...
      const [entries, childCompilation] = await new Promise(
        (resolve, reject) => {
      ...
      const outputName = compilation.hooks.assetPath.call(
...
function buildChildCompilerName(context, chunkName, manifestPath) {
  ...
  return `IconduitWebpackHtmlPlugin for manifest ${JSON.stringify(
    shortestPath,
  )}`;
}

function translateTag(data) {
  const { attributes, children, isSelfClosing, tag } = data;
